  ...
  <Panel caption="Results">
    <div class="flex gap-2 flex-wrap">
      ...
      <div
        class="w-32 h-32 border border-transparent border-t-slate-50 text-center shadow-lg bg-slate-100 rounded-lg"
      >
        <h3 class="text-xl mt-6">Wager Req.</h3>
        <div class="mt-4">
          {{ wagerRequirement }}
        </div>
      </div>
      ...
      <WagerRequirementWinRateGraph ref="graph"></WagerRequirementWinRateGraph>
    </div>
  </Panel>
</template>
...
function findStabilizationPoint(
  data: any,
  threshold = 0.05,
  stablePoints = 200
) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (count == stablePoints) {
      return i - stablePoints + 1;
    }
  }
  return -1;
}

function findAverageAfterStabilization(data: any) {
  const stabilizationPoint = findStabilizationPoint(data);

  if (stabilizationPoint === -1) return null;

  const sum = data
    .slice(stabilizationPoint)
    .reduce((acc: any, val: any) => acc + val, 0);
  return sum / (data.length - stabilizationPoint);
}

const props = defineProps(["config"]);
const emit = defineEmits(["random"]);

const wagerRequirement = ref(52);

const graph = ref();
const winRates = ref(<Record<any, any>>{});
const graphStats = (rate: number) => {
  winRates.value[wagerRequirement.value] =
    winRates.value[wagerRequirement.value] || [];

  winRates.value[wagerRequirement.value].push(rate);

  const stabilized = findAverageAfterStabilization(
    winRates.value[wagerRequirement.value]
  );

  if (null !== stabilized) {
    graph.value?.add(stabilized, wagerRequirement.value);

    if (stabilized > props.config.mp_target_win_rate.value) {
      wagerRequirement.value++;
    } else {
      wagerRequirement.value--;
    }
...
watch(props.config, () => {
  ...
  results.loses = 0;
  winRates.value = {};
  wagerRequirement.value = 52;
  graph.value?.reset();
});

// watch(wagerRequirement, () => {
// results.total = 0;
// results.wins = 0;
// results.loses = 0;
// winRates.value = {};
...
const simulate = () => {
  ...
  const wager_requirement = deposit.times(wagerRequirement.value).toNumber();
  ...
  while (true) {
    ...
    if (wager >= wager_requirement) {
      if (deposit.toNumber() >= parseInt(props.config.mp_deposit.value)) {
        results.total++;
        results.wins++;
        graphStats(Number(((results.wins / results.total) * 100).toFixed(2)));
        break;
      } else {
        results.total++;
        results.loses++;
